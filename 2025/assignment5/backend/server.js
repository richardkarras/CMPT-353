...
const nano = require("nano")(/*process.env.COUCHDB_URL ||*/ 'http://admin:password@a5couch:5984');
const cors = require("cors");
const corsOptions = {
  origin: ["http://localhost:80","http://a5react:3000","http://localhost:3000"],
};

// const username = process.env.COUCHDB_USER;
// const password = process.env.COUCHDB_PASSWORD;
const port = process.env.NODEPORT || 81;
...
/* alldata ** GET method **
-purpose: retrieve all posts, responses and views from database
*/
app.get("/alldata", async (req, res) => {
  try {
    const allData = await db.list({ include_docs: true });
    res.json(allData.rows.map((row) => row.doc));
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
});
...
app.get("/allposts", async (req, res) => {
  try {
    await db.view('all_posts', 'all_posts', { include_docs: true })
      .then(function(data) {
...
app.get("/allresponses", async (req,res) => {
  try {
    const allResponses = await db.view('all_responses', 'all_responses', {include_docs: true})
